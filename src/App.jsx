...
function App() {
  return (
    ...
    <div className="w-screen min-h-screen flex flex-col items-center justify-center pt-40 bg-custom1 text-center">
      ...
      <div style={{backgroundImage: `url(${Pic6})`}} className="img w-full h-full rounded-2xl relative main">
      <div className="text-white shadow-x font-bold text-2xl w-full flex items-center justify-around absolute bottom-0 py-5 bg-opacity-40 bg-black opacity-0 rounded-2xl on-hover transition-x w-full h-full flex flex-col">
        <p>Black Nike</p>
        <p>$100</p>
      </div>
      </div>
      <div style={{backgroundImage: `url(${Pic7})`}} className="img w-full h-full rounded-2xl relative main">
      <div className="text-white shadow-x font-bold text-2xl w-full flex items-center justify-around absolute bottom-0 py-5 bg-opacity-40 bg-black opacity-0 rounded-2xl on-hover transition-x w-full h-full flex flex-col">
        <p>Gray Nike</p>
        <p>$100</p>
      </div>
      </div>
      <div style={{backgroundImage: `url(${Pic8})`}} className="img w-full h-full rounded-2xl relative main">
      <div className="text-white shadow-x font-bold text-2xl w-full flex items-center justify-around absolute bottom-0 py-5 bg-opacity-40 bg-black opacity-0 rounded-2xl on-hover transition-x w-full h-full flex flex-col">
        <p>Colorful Nike</p>
        <p>$100</p>
      </div>
    ...
    <div className="w-screen bg-custom1 text-5xl lg:text-7xl text-center flex items-center justify-center font-bold pb-36 pt-60">
      <h1 className="italic pl-3 pr-9 py-6 rounded-3xl w-2/3 shadow-x text-white">Use Code <span className="text-emerald-500 font-bold text-7xl lg:text-9xl">Rx16ew</span> To Get A <span className="text-red-600 font-bold text-7xl lg:text-9xl">50%</span> Off</h1>
    </div>
